...
def clearCache():
    ...
    if os.path.exists(cachePath)==True:    
        for root, dirs, files in os.walk(cachePath):
            ...
            if file_count > 0:
                ...
                if dialog.yesno("Borrar Kodi Cache", str(file_count) + " Archivos Encontrados", "Lo Eliminamos?"):
    ...
    if os.path.exists(tempPath)==True:    
        for root, dirs, files in os.walk(tempPath):
            ...
            if file_count > 0:
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Borrar Kodi Archivos Temporales", str(file_count) + " Archivos Encontrados", "Lo Eliminamos?"):
                    for f in files:
                        try:
                            if (f == "xbmc.log" or f == "xbmc.old.log"): continue
                            os.unlink(os.path.join(root, f))
...
def deleteThumbnails():
    ...
    try:
		...
		pass
    dialog.ok("Resetea Kodi", "Por Favor reinicia kodi para restablecer las imagenes")
