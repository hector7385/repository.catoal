...
if count > maxpackage_zips or int(total_sizetext) > filesize:
	choice2 = xbmcgui.Dialog().yesno("[COLOR=blue]Autolimpiar[/COLOR]", 'La Carpeta packages tiene[COLOR yellow]' + str(total_sizetext) +' MB [/COLOR] - [COLOR red]' + str(count) + '[/COLOR] zips', 'La carpeta se puede limpiar sin problemas para ahorrar espacio ...', 'las eliminamos?', yeslabel='Si',nolabel='No')
	if choice2 == 1:
...
if int(total_sizetext2) > filesize_thumb:
	choice2 = xbmcgui.Dialog().yesno("[COLOR=blue]AutoLimpiar[/COLOR]", 'La Carpeta imagenes tiene[COLOR yellow]' + str(total_sizetext2) + ' MB   [/COLOR]', 'La carpeta se puede limpiar sin problemas para ahorrar espacio ...', 'las eliminamos?', yeslabel='Si',nolabel='No')
	if choice2 == 1:
		control.deleteThumbnails()

xbmc.executebuiltin('XBMC.Notification(%s, %s, %s, %s)' % ('Limpiar Kodi',  'Packages: '+ str(total_sizetext) +  ' MB'  ' - Imagenes: ' + str(total_sizetext2) + ' MB' , '5000', iconpath))