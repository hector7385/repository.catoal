...
def index():
    ...
    listUrl = ['/index.php?page=categorias&url=peliculas&letter=',
               '/index.php?page=categorias&url=peliculas-latino&letter',
               '/index.php?page=categorias&url=estrenos-de-cine&letter=',
               '/index.php?page=categorias&url=peliculas-hd&letter=',
               '/index.php?page=categorias&url=peliculas-3d&letter=',
               '/index.php?page=categorias&url=otras-peliculas&letter=',
               '/index.php?page=categorias&url=peliculas-vo&letter=',
               ]
...
def nextPage(url="", page="1", showSeasons='True'):
    if ('/buscar?q=' in url):
        payload = {'q': url.replace('/buscar?q=', '')}
        url = settings.value["urlAddress"] + '/buscar'

        # Read
        settings.log(url)
        response = browser.post(url, data=payload, verify=False)
        soup = bs4.BeautifulSoup(response.text)
        links = soup.select("ul.buscar-list li a")
    else:
        url = settings.value["urlAddress"] + url if url.startswith('/') else url
        url += '&pg=' + page
        settings.log(url)
        ...
        if 'series' in url:
            ...
            links = soup.select("ul.pelilist li a")
    ...
    for a in links:
        titles.append(
                a.get("title", a.text).replace("Descargar", "").replace("Serie En", "").replace("Serie Vo", "").replace(
                        "Serie", ""))
