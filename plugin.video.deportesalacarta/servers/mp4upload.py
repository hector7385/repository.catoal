...
def get_video_url(page_url, premium=False, user="", password="", video_password=""):
    logger.info("deportesalacarta.servers.mp4upload get_video_url(page_url='%s')" % page_url)

    data = scrapertools.cache_page(page_url)
    logger.info("deportesalacarta.servers.mp4upload data=" + data)
    media_url = scrapertools.find_single_match(data, '"file": "(.+?)"')
    logger.info("deportesalacarta.servers.mp4upload media_url=" + media_url)
    ...
    for video_url in video_urls:
        logger.info("deportesalacarta.servers.mp4upload %s - %s" % (video_url[0], video_url[1]))
