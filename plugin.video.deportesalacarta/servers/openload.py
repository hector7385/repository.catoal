...
def test_video_exists(page_url):
    logger.info("deportesalacarta.servers.openload test_video_exists(page_url='%s')" % page_url)
...
def get_video_url(page_url, premium=False, user="", password="", video_password=""):
    logger.info("deportesalacarta.servers.openload url=" + page_url)
    video_urls = []
    ...
    try:
        ...
        import traceback
        logger.info("deportesalacarta.servers.openload "+traceback.format_exc())
        # Falla el método, se utiliza la api aunque en horas punta no funciona
        videourl, extension = get_link_api(page_url)
    ...
    for video_url in video_urls:
        logger.info("deportesalacarta.servers.openload %s - %s" % (video_url[0],video_url[1]))
...
def decode_hidden(text, number):
    ...
    text = text.replace("&anp;", "&").replace("&amq;", "&")#.replace("&quot9", '"').replace("&lt9", '<') \
