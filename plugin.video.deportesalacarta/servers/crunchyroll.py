...
def test_video_exists( page_url ):
    logger.info("deportesalacarta.servers.crunchyroll test_video_exists(page_url='%s')" % page_url)
...
def get_video_url(page_url, premium = False, user="", password="", video_password=""):
    logger.info("deportesalacarta.servers.crunchyroll url="+page_url)
    ...
    data = scrapertools.downloadpage(url, post, headers)
    

    if "<msg>Media not available</msg>" in data:
        data = scrapertools.downloadpage(proxy+url, post, headers)
    ...
    try:
        ...
        data_sub = scrapertools.downloadpage(link_sub.replace("&amp;", "&"))
    ...
    for video_url in video_urls:
        logger.info("deportesalacarta.servers.crunchyroll %s - %s" % (video_url[0],video_url[1]))
...
def find_videos(data):
    ...
    logger.info("deportesalacarta.servers.crunchyroll find_videos #"+patronvideos+"#")
...
def login(data):
    ...
    data = scrapertools.downloadpage(login_page)
    if not "<title>Redirecting" in data:
        ...
        data = scrapertools.downloadpage(login_page, post)
