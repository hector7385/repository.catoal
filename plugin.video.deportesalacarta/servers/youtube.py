...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    ...
    # if "ext" in result and "url" in result:
        # for child in result["formats"]:
            # logger.info(str(child))
            # if child["ext"] != "f4m" and not "ignore.mediaset.es" in child["url"]:
                # title = child["format"].encode('utf-8')
                # video_urls.append(["%s" % title, child['url'].encode('utf-8')])
    # return video_urls
