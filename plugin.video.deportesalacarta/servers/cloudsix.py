...
def test_video_exists( page_url ):
    logger.info("deportesalacarta.servers.cloudsix test_video_exists(page_url='%s')" % page_url)
    
    return False,"Este servidor no es compatible con pelisalacarta"

def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("deportesalacarta.servers.cloudsix get_video_url(page_url='%s')" % page_url)
    video_urls = []
    return video_urls
