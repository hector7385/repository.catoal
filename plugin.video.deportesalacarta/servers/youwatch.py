...
def test_video_exists(page_url):
    logger.info("deportesalacarta.servers.youwatch test_video_exists(page_url='%s')" % page_url)
...
def get_video_url(page_url, premium=False, user="", password="", video_password=""):
    logger.info("deportesalacarta.servers.youwatch get_video_url(page_url='%s')" % page_url)
    ...
    for video_url in video_urls:
        logger.info("deportesalacarta.servers.youwatch %s - %s" % (video_url[0], video_url[1]))
