...
def test_video_exists( page_url ):
    logger.info("deportesalacarta.servers.turbovideos test_video_exists(page_url='%s')" % page_url)
    return True,""

def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("deportesalacarta.servers.turbovideos url="+page_url)
    
    if not "embed" in page_url:
        page_url = page_url.replace("http://turbovideos.net/","http://turbovideos.net/embed-") + ".html"
    
    data = scrapertools.cache_page( page_url )
    logger.info("deportesalacarta.servers.turbovideos data="+data)
    ...
    for media_url,label in media_urls:
        ...
        if not media_url.endswith("srt"):
            video_urls.append( [ scrapertools.get_filename_from_url(media_url)[-4:]+" "+label+" [turbovideos]",media_url])
