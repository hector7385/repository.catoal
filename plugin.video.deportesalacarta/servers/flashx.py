...
def test_video_exists(page_url):
    logger.info("deportesalacarta.servers.flashx test_video_exists(page_url='%s')" % page_url)
...
def get_video_url(page_url, premium=False, user="", password="", video_password=""):
    logger.info("deportesalacarta.servers.flashx url=" + page_url)
    ...
    for media_url, label in media_urls:
        if media_url.endswith(".srt") and label == "Spanish":
            try:
                ...
                import traceback
                logger.info("deportesalacarta.servers.flashx Error al descargar el subtítulo: "+traceback.format_exc())
    ...
    for video_url in video_urls:
        logger.info("deportesalacarta.servers.flashx %s - %s" % (video_url[0], video_url[1]))
