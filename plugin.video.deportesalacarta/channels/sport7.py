        ...
        for fecha, bloque_partidos in matchespartidos:
            ...
            for thumbnail, hora, url, title in matches:
                ...
                if day in date and int(check_match_hour) == int(check_today_hour) and int(check_match_minute) <= int(check_today_minute) or day in date and int(check_match_hour) < int(check_today_hour) and int(check_today_hour)< int(check_match_end_live) :
                    ...
                    mes = scrapertools.get_match(date, 'de ([A-z]+) ')
                ...
                print "amoooo"
                print url
                itemlist.append( Item(channel=__channel__, title="     "+title,action="enlaces",url=url,thumbnail =thumbnail,fanart ="http://s6.postimg.org/uo85sphn5/sport7fanart.jpg",extra=extra,fulltitle = fulltitle,deporte="futbol", evento=evento, date=dia_mes, time=time,context="info_partido",folder=True) )
