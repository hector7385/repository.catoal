...
if not BOOKMARK_PATH.upper().startswith("SMB://"):
    if BOOKMARK_PATH.startswith("special://"):
        # logger.info("deportesalacarta.channels.favoritos Se esta utilizando el protocolo 'special'")
...
def mainlist(item):
    logger.info("deportesalacarta.channels.favoritos mainlist")
    itemlist = []
...
def savebookmark(canal=CHANNELNAME, titulo="", url="", thumbnail="", server="", plot="", fulltitle="",
    ...
    logger.info("deportesalacarta.channels.favoritos savebookmark(path="+savepath+")")
    ...
    if len(ficheros) > 0:
        ...
        filenumber = 1
        for fichero in ficheros:
            logger.info("deportesalacarta.channels.favoritos fichero="+fichero)
            try:
    ...
    logger.info("deportesalacarta.channels.favoritos savebookmark filename="+filename)
...
def deletebookmark(fullfilename, deletepath=BOOKMARK_PATH):
    logger.info("deportesalacarta.channels.favoritos deletebookmark(fullfilename="+fullfilename+",deletepath="+deletepath+")")
    ...
    if not samba.usingsamba(deletepath):
        ...
        logger.info("deportesalacarta.channels.favoritos filename="+filename)
        logger.info("deportesalacarta.channels.favoritos deletepath="+deletepath)
