...
def enlaces(item):
    ...
    xbmc.log("estoquees")
    data = scrapertools.cachePage(item.url)
    data = re.sub(r".<!-- Table Body -->|<!-- VLC -->|<!-- livetable -->|<!-- <meta|<!-- <script|No VLC Links available for this match|<!-- Analytics -->","",data)
    xbmc.log("pepote")
    xbmc.log(data)
    patronlinks = '<!-- (.*?) -->.*?<img src="([^"]+)".*?(.*?)<!-- .*? -->'
    ...
    for tipo,image, bloque_links in matcheslinks:
        ...
        xbmc.log("pacorro2")
        xbmc.log(str(tiempo_partido))
        if '<td width="100%" align="left" valign="middle" colspan="3" ><strong>No' in bloque_links and tiempo_actual<= tiempo_partido:
            xbmc.log("lopera")
            if tipo == "Sopcast":
               thumbnail ="http://s6.postimg.org/u8h4qcuc1/nosopcast.png"
            if tipo == "Acestream":
        ...
        for canal,idioma, link in matches:
            ...
            if "More Acestream Sport" in title:
                continue
            if '<td width="100%" align="left" valign="middle" colspan="3" ><strong>No' in link and  tiempo_actual > tiempo_partido:
                xbmc.log("lopera2")
