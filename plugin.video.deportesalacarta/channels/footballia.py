...
def login():
    ...
    try:
        ...
        try:
            os.remove(os.path.join(config.get_data_path(), 'cookies', 'footballia.net.dat'))
        except:
            pass

        data = scrapertools.cachePage("http://footballia.net/es")
        token = scrapertools.find_single_match(data, 'content="([^"]+)" name="csrf-token"')
        post = "utf8=✓&authenticity_token=%s&user[email]=%s&user[password]=%s&user[remember_me]=0&user[remember_me]=1&commit=Entra" % (token, user, password)
