...
def lista2(item):
    ...
    data += scrapertools.cachePage("http://tuchkatv.ru/hd/sportivnye-hd/")
    data = re.sub(r"\n|\r|\t|\s{2}|&nbsp;","",data)
    ...
    patron = '<div class=maincont>(<div class=gray>|).*?<img (?:title="(.*?)\s*(?:online|Online|").*?src=(.*?png)|src=(.*?png).*?title="(?:"|(.*?)\s*(?:online|Online)))'
