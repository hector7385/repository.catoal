...
# ------------------------------------------------------------
# This file is part of deportesalacarta 4.
#
# deportesalacarta 4 is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# deportesalacarta 4 is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with deportesalacarta 4.  If not, see <http://www.gnu.org/licenses/>.
# ------------------------------------------------------------
        ...
        if not self.data_home:
            ...
            try:
                if not self.url_home:
                    data = scrapertools.downloadpageWithoutCookies("http://www.transfermarkt.es/schnellsuche/ergebnis/schnellsuche?query=%s&x=0&y=0" % urllib.quote(self.home_name_tf.replace("%2B", "+")))
                    ...
                    if self.home_teams_transf:
                        ...
                        self.data_home = scrapertools.downloadpageWithoutCookies(self.url_home)
            ...
            try:
                if not self.url_away:
                    data = scrapertools.downloadpageWithoutCookies("http://www.transfermarkt.es/schnellsuche/ergebnis/schnellsuche?query=%s&x=0&y=0" % urllib.quote(self.away_name_tf.replace("%2B", "+")))
                    bloque = scrapertools.find_single_match(data, '<div class="table-header">Buscar resultados: Clubes(.*?)<div class="keys"')
                    ...
                    if self.away_teams_transf:
                        ...
                        self.data_away = scrapertools.downloadpageWithoutCookies(self.url_away)
                    ...
                    if status != "-:-":
                        ...
                        if (fecha_actual < fecha and fecha_actual >= fecha - datetime.timedelta(hours=1)) or (fecha_actual >= fecha and fecha_actual - datetime.timedelta(hours=2, minutes=30) <= fecha):
                            if not self.data_directo:
                                ...
                                self.data_directo = scrapertools.downloadpage(self.url_directo)
...
class Thumb(xbmcgui.WindowDialog):
    ...
    def Start( self, *args, **kwargs):
        ...
        data = scrapertools.downloadpage(self.url)
    ...
    def Start(self, url, escudo1, name1, escudo2, name2, result, data="", fonts=""):
        ...
        if not self.once_local:
            try:
                ...
                if url_formacion:
                    data_form = scrapertools.downloadpage("http://www.transfermarkt.es"+url_formacion)
                    data_form = re.sub(r"\n|\r|\t|\s{2}|&nbsp;","", data_form)
        ...
        if url_estadistica:
            ...
            data_stat = scrapertools.downloadpage(url_estadistica)
            data_stat = re.sub(r"\n|\r|\t|\s{2}|&nbsp;","", data_stat)
        ...
        try:
            ...
            if not bloque:
                ...
                if url_ticker:
                    self.data_t = scrapertools.downloadpage("http://www.transfermarkt.es"+url_ticker)
    ...
    def Start(self, url, logo, name, season="", fonts=""):
        ...
        try:
            ...
            if url_pichichi:
                ...
                data_pichichi = scrapertools.downloadpage("http://www.transfermarkt.es"+url_pichichi+"/plus/1")
        ...
        try:
            ...
            if url_zamora:
                ...
                data_zamora = scrapertools.downloadpage("http://www.transfermarkt.es"+url_zamora+"/plus/1")
    ...
    def onClick(self, id):
        ...
        try:
            if id == self.search.getId():
                ...
                if texto:
                    texto = texto.strip()
                    data = scrapertools.downloadpage("http://www.transfermarkt.es/schnellsuche/ergebnis/schnellsuche?query=%s&x=0&y=0" % urllib.quote(texto.replace("%2B", "+")))
...
def elegir_equipo(self):
    ...
    # Búsqueda en transfermarkt
        ...
        if name == "busqueda":
            ...
            if texto_busqueda:
                texto_busqueda = texto_busqueda.strip()
                data = scrapertools.downloadpage("http://www.transfermarkt.es/schnellsuche/ergebnis/schnellsuche?query=%s&x=0&y=0" % urllib.quote(texto_busqueda.replace("%2B", "+")))
                bloque = scrapertools.find_single_match(data, '<div class="table-header">Buscar resultados: Clubes(.*?)<div class="keys"')
                ...
                data = scrapertools.downloadpage(item.getProperty("url"))
                bloque = scrapertools.find_single_match(data, '<div class="table-header">Buscar resultados: Clubes(.*?)<div class="keys"')
                lista_transf = scrapertools.find_multiple_matches(bloque, '<td class="zentriert suche-vereinswappen"><img src="([^"]+)".*?alt="([^"]+)".*?href="([^"]+)"')
                next_page = scrapertools.find_single_match(bloque, '<li class="naechste-seite"><a href="([^"]+)"')
                if next_page:
...
def canticos(self, name="", type=""):
    ...
    if not xbmc.Player().isPlaying() and not self.songs:
        data = scrapertools.downloadpage("http://fanchants.es/search/?query=%s&site=5" % urllib.quote(name))
        ...
        try:
            self.data_chant = scrapertools.downloadpage(audio)
        except:
