...
def agenda(item):
    ...
    dia, mes, bloque = scrapertools.find_single_match(data, '(?i)emisiones</span>.*?>(\d+) (?:(?i)de) (\w+)[^<]+</span></span>(.*?)<div class=block_full')
...
def findlives(item):
    ...
    patron = '<a href=(acestream://[^>]+)>.*?>-.*?</span>.*?>(-[^<]+)<\/span>'
    matches = scrapertools.find_multiple_matches(data, patron)
    if matches:
...
def futbol(item):
    ...
    data = translate(item.url)
    calidades = scrapertools.find_multiple_matches(data, '(?i)Calidad de.*?>(?:<b>|)([^<]+)(?:</b>|)</span></span>')
    patron = '(?:<div id=allEntries>|<h4 class=viewn_title>).*?href=.*?u=([^&]+)\S*>.*?alt="([^"]+)".*?src=(.*?)\s+'
    matches = scrapertools.find_multiple_matches(data, patron)
    i = 0
    for url, title, thumb in matches[:20]:
        thumb = host + thumb
        title = re.sub(r'(?i)ronda|tour', 'jornada', title).replace("éter", "programa").replace("turísticos", "")
        try:
            title = "[COLOR blue]%s[/COLOR] [COLOR red][%s][/COLOR]" % (title, calidades[i])
        except:
