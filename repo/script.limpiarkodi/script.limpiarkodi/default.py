...
def mainMenu():
    ...
    addItem('Limpiar Cache','url', 1,icon)
    addItem('Borrar Imagenes', 'url', 2,icon)
    addItem('Purgar Packages', 'url', 3,icon)
...
def clearCache():
    ...
    if os.path.exists(cachePath)==True:    
        for root, dirs, files in os.walk(cachePath):
            ...
            if file_count > 0:
                ...
                if dialog.yesno("Borrar Kodi Cache", str(file_count) + " Archivos Encontrados", "Los Eliminamos?"):
    ...
    if os.path.exists(tempPath)==True:    
        for root, dirs, files in os.walk(tempPath):
            ...
            if file_count > 0:
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Borrar Kodi Archivos Temporales", str(file_count) + " Archivos Encontrados", "Los Eliminamos?"):
                    for f in files:
                        try:
                            if (f == "xbmc.log" or f == "xbmc.old.log"): continue
                            os.unlink(os.path.join(root, f))
    ...
    if xbmc.getCondVisibility('system.platform.ATV2'):
        ...
        for root, dirs, files in os.walk(atv2_cache_a):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Borrar ATV2 Cache", str(file_count) + " Archivos Encontrados'", "Los Eliminamos?"):
        ...
        for root, dirs, files in os.walk(atv2_cache_b):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Borrar ATV2 Cache Files", str(file_count) + " archivos en 'LocalAndRental'", "Los Eliminamos?"):
        ...
        if os.path.exists(clear_cache_path)==True:    
            for root, dirs, files in os.walk(clear_cache_path):
                ...
                if file_count > 0:
                    ...
                    dialog = xbmcgui.Dialog()
                    if dialog.yesno("Manager",str(file_count) + "%s Archivos Encontrados"%(entry.name), "Los Eliminamos?"):
                        for f in files:
                            os.unlink(os.path.join(root, f))
                        for d in dirs:
                            shutil.rmtree(os.path.join(root, d))
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("Limpiado", "Eliminados archivos cache")
...
def deleteThumbnails():
    ...
    if os.path.exists(thumbnailPath)==True:  
            dialog = xbmcgui.Dialog()
            if dialog.yesno("Borrar Imagenes", "Esta Opcion Borra las Imagenes", "Estas Seguro?"):
...
def purgePackages():
    ...
    for root, dirs, files in os.walk(purgePath):
            file_count = 0
            file_count += len(files)
    if dialog.yesno("Borrar Package", "%d packages Encontrados."%file_count, "Los Eliminamos?"):  
        for root, dirs, files in os.walk(purgePath):
            file_count = 0
            file_count += len(files)
            if file_count > 0:            
                ...
                for d in dirs:
                    shutil.rmtree(os.path.join(root, d))
                dialog = xbmcgui.Dialog()
                dialog.ok("Limpiado", "Eliminamdos Packages")
            else:
                dialog = xbmcgui.Dialog()
